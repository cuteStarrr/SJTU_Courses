...
class ARP_PROXY_13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if ipv_4:
            ...
            if dpid == 2:
                if in_port == 1:
                    self.add_group_flow(datapath, 1, parser.OFPMatch(in_port=in_port, eth_dst=dst), 2)
                    self.add_group_flow(self.dp[1], 1, parser.OFPMatch(in_port=in_port, eth_dst=src), 1)
                    
                    data = None
                    if msg.buffer_id == ofproto.OFP_NO_BUFFER:
                        data = msg.data
                    out = parser.OFPPacketOut(datapath=datapath, buffer_id=msg.buffer_id,
                                    in_port=in_port, actions=[parser.OFPActionOutput(2)], data=data)
            ...
            if dpid == 1:
                if in_port == 1:
                    self.add_group_flow(datapath, 1, parser.OFPMatch(in_port=in_port, eth_dst=dst), 1)
                    self.add_group_flow(self.dp[2], 1, parser.OFPMatch(in_port=in_port, eth_dst=src), 2)
        ...
        if dst in self.mac_to_port[dpid]:
            if case < 5:
                out_port = self.mac_to_port[dpid][dst][0]
            else:
                out_port = self.mac_to_port[dpid][dst][-1]
        else:
            print(self.mac_to_port[dpid])
            out_port = ofproto.OFPP_FLOOD
            print("Flood")
        ...
        #print("mac to port is:", self.mac_to_port)
