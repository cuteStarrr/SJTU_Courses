...
from ryu.lib.packet import packet, ethernet, arp, ipv6, ipv4, icmp
...
class ARP_PROXY_13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if ipv_4:
            ...
            if dpid == 2:
                if in_port == 1:
                    self.add_group_flow(datapath, 2, parser.OFPMatch(in_port=in_port, eth_dst=dst))
                    self.add_group_flow(self.dp[1], 2, parser.OFPMatch(in_port=in_port, eth_dst=src))
            ...
            if dpid == 1:
                if in_port == 1:
                    self.add_group_flow(datapath, 2, parser.OFPMatch(in_port=in_port, eth_dst=dst))
                    self.add_group_flow(self.dp[2], 2, parser.OFPMatch(in_port=in_port, eth_dst=src))
