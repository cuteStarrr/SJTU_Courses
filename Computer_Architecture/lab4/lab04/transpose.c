...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
    ...
    stepx = blocksize;
    stepy = blocksize;
    num = (n + blocksize - 1) / blocksize;
    for (int i = 0; i< num; ++i) {
        if(i == num - 1) stepx = n - (num - 1) * blocksize;
        //else stepx = blocksize;
        for(int j = 0; j < num; ++j) {
            if(j == num - 1) stepy = n - (num - 1) * blocksize;
            else stepy = blocksize;
            for (int x = 0; x < stepx; ++x) {
                for (int y = 0; y < stepy; ++y) {
                    dst[j * blocksize + y + (x + i * blocksize) * n] = src[x + i * blocksize + (y + j * blocksize) * n];
                }
            }
        }
    }
