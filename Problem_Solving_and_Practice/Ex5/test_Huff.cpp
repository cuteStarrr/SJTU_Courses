...
HuffTree :: HuffTree(map<string, int> ele_fre) {
    ...
    for(map<string, int> :: iterator times = ele_fre.begin(); times != ele_fre.end(); ++times) {
    ...
    for(int i = 0; i < symbol_num; ++i) {
        ...
        cout << root[i + symbol_num].symbol << ' ' << root[i + symbol_num].freq << endl;
...
int main() {
    ...
    for(map<string, int> :: iterator times = frequency.begin(); times != frequency.end(); ++times) {
        cout << times -> first << ' ' << times -> second << '\n';
    }
