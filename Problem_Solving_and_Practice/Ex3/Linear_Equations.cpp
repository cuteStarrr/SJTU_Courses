...
string LinearEquations::OutputAnswer() {
    ...
    string output = "";
    map<string, string> ans_map;
    for(int i = 1; i <= symbol_num; ++i) {
        ans_map[symbol[i]] = FloatStr(matrix[i][0]);
    }
    for (map<string, string>::iterator iter = ans_map.begin(); iter != ans_map.end(); ++iter) {  
        output = output + iter->first + ":" + iter->second + "\n";
    } 
...
bool LinearEquations::FindAnswer() {
    ...
    if(abs(matrix[RA + 1][0]) > 0.00001) {error1 = 1; return false;}
    else {
...
Term NumTerm(string str, int & pos) {
    ...
    if(str[pos] != '*') throw IllegalEquation(str); // 跳过*
    ++pos;
    int ch = int(str[pos]);
    if(ch >= 97 && ch <= 122) { // 字母开头
        var = GetString(str, pos); // pos变为接下来要处理的字符的位置
    }
    else throw IllegalEquation(str);
    return Term(val, var);
}
...
string GetMatrix(int & variable_num, int & line, string* variables, double coef[][MAXNUM], int & in_num, string & legal) {
    ...
    while(getline(cin,str)) {
        ...
        try {
            ...
            if(equal_pos == string::npos) throw IllegalEquation(str);
            else {
                ++equal_pos; // 跳过=
                int tmp = GetFloat(str,equal_pos);
                if(equal_pos < size) throw IllegalEquation(str);
            }
            int pos = 0;
            while(str[pos] != '=') {
