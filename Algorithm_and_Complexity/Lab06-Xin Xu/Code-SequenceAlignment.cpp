...
int editDistanceDP(string &X, string &Y, int xLow, int yLow, int xHigh, int yHigh, const int &delta, bool reverse, vector<int> &dist) {
	...
	*/
	...
	for(i=xLow;i<=xHigh;++i){
		dist.push_back((i-xLow)*delta);
	}

	for(j=yLow;j<=yHigh;++j){
		tmp.push_back((j-yLow+1)*delta);
		for(i=xLow;i<=xHigh;++i){
			Vmin = dist[i-xLow] + alpha(X[i - 1], Y[j - 1]);
			Vmin = min(Vmin, tmp.back() + delta);
			Vmin = min(Vmin, dist[i-xLow+1] + delta);
			tmp.push_back(Vmin);
		}
		dist=tmp;
	}
    ...
    for(i=xHigh;i>=xLow;--i){
		dist.push_back((xHigh-i)*delta);
	}
	for(j=yHigh-1;j>=yLow;--j){
		tmp.push_back((yHigh-j)*delta);
		for(i=xHigh-1;i>=xLow;--i){
			Vmin = dist[xHigh-i-1] + alpha(X[i], Y[j]);
			Vmin = min(Vmin, tmp.back() + delta);
			Vmin = min(Vmin, dist[xHigh-i] + delta);
			tmp.push_back(Vmin);
		}
		dist=tmp;
	}
